...
import {
  ...
  validateImportData,
  importAllData,
  // US-080: Achievement imports
  getAchievements,
  saveAchievements,
  updateAchievement,
  unlockAchievement,
  getUnlockedAchievementsCount,
  // US-081: Daily Challenges imports
  getDailyChallenges,
  saveDailyChallenges,
 ...
 */
function selectGoalByIndex(index) {
  const visibleGoals = getVisibleGoals();
 ...
 * - Counter: (no primary action via Space, use +/- for increment/decrement)
 * - Checkbox: toggle completion
 */
function performPrimaryActionOnSelectedGoal() {
  const goal = getSelectedGoal();
  if (!goal) return;

  switch (goal.type) {
    case GOAL_TYPES.TIMER:
      handleTimerToggle(goal.id);
      break;
    case GOAL_TYPES.CHECKBOX:
      handleCheckboxToggle(goal.id);
      break;
    case GOAL_TYPES.COUNTER:
      // For counter, Space increments (consistent with being the "primary" action)
      handleCounterIncrement(goal.id);
      break;
  }
}
 ...
 */
function incrementSelectedGoal() {
  const goal = getSelectedGoal();
 ...
 */
function decrementSelectedGoal() {
  const goal = getSelectedGoal();
  if (!goal || goal.type !== GOAL_TYPES.COUNTER) return;
  handleCounterDecrement(goal.id);
}
 ...
 */
function renderKeyboardShortcutsHelp() {
  return `
...
// =============================================================================
// US-080: Achievements and Badges Screen
// =============================================================================
 ...
 */
function renderAchievementBadge(definition) {
 ...
 */
async function checkAchievements(trigger, context = {}) {
  const history = await getHistory();
  const activityLog = await getActivityLog();
 ...
 */
function countPerfectDays(history, goals) {
 ...
 */
function showAchievementUnlockNotification(definition) {
 ...
 */
async function initializeDailyChallenge(existingData) {
  const today = getTodayDateString();
 ...
 */
function attachDailyChallengeListeners(screen) {
 ...
 */
async function updateChallengeProgress(eventType, context = {}) {
  const challengesEnabled = state.settings?.dailyChallengesEnabled !== false;
 ...
 */
function renderChallengeStatsSection() {
  const challengesEnabled = state.settings?.dailyChallengesEnabled !== false;
  if (!challengesEnabled) return '';
        ...
        <div class="challenge-history">
          ...
          <div class="challenge-history-list">
            ${recentHistory.map(entry => {
              ...
              const status = entry.completed ? 'completed' : entry.skipped ? 'skipped' : 'expired';
...
function renderReportsScreen() {
  ...
  attachShareReportsListeners(screen);
